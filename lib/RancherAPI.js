        ...
        return this.getService(options.serviceName).then(function (svc) {
            ...
            if (options.confirmBefore && svc.state === 'upgraded') {
                ...
                return self.post(svc.actions.finishupgrade, {}).then(function(result) {
                    ...
                    return self.waitForActive(options.serviceName).then(function (svc) {
                        LOG("Starting upgrade...");
                        return self.inServiceUpgrade(options);
                    });
                });
            } else if (svc.state !== 'active' && !svc.actions.upgrade) {
                ...
                if (options.imageTag) {
                    uuid += ":" + options.imageTag;
                }

                (svc.launchConfig.imageUuid = uuid),
                    LOG(
                    "Changed service image uuid to",
                    svc.launchConfig.imageUuid
                    );

                svc.launchConfig.environment = Object.assign(
                    svc.launchConfig.environment || {},
                    options.environment
                );
                ...
                return self.post(svc.actions.upgrade, {
                  ...
                  .then(function (result) {
                    ...
                    return self.waitForUpgrade(
                      options.serviceName,
                      options.confirmAfter
                    );
                  });
            }
        })
